...
export class Interpreter {
    ...
    visitClassStmt(stmt) {
        ...
        for (const method of stmt.methods) {
            const func = new FunctionR(method, this.environment, method.name.value == "init");
            methods.set(method.name.value, func);
        }
        const klass = new ClassR(stmt.name.value, superclass, methods);
        if (superclass != null) {
    ...
    visitFunctionStmt(stmt) {
        ...
        const _function = new FunctionR(stmt, this.environment, false);
    ...
    visitPostExpr(expr) {
        ...
        if (expr.name.constructor.name == "Variable") {
            const distance = this.locals.get(expr.name);
            if (distance != null) {
    ...
    visitSuperExpr(expr) {
        ...
        const method = superclass.findMethod(expr.method.value);
        if (method == null) {
