...
class WHMenuItem extends HTMLElement {
    ...
    constructor() {
        ...
        const slot: HTMLSlotElement = this.shadowRoot.querySelector(`slot[name="group"]`);
...
class WHMenu extends HTMLElement {
    ...
    handleEvent(event) {
        if (event.type == "click") {
            const target: any = event.target.closest("wh-menu-item[action]");
			
            if (target?.action) {
                $(this).fire("app-action", target?.action);
                return;
            }
        }
    }
